...
from cs336_basics.model import Linear, EmbeddingModule, Rmsnorm, Swiglu, RotaryPositionalEmbedding, softmax, scaled_dot_product_attention, MultiheadSelfAttention, TransformerBlock, TransformerLm
...
def run_embedding(
    ...
    embedding = EmbeddingModule(vocab_size, d_model, device=weights.device, dtype= weights.dtype)
...
def run_multihead_self_attention_with_rope(
    ...
    multihead.v_proj.weight.data = v_proj_weight
    multihead.output_proj.weight.data = o_proj_weight
    return multihead.forward(in_features, RotaryPositionalEmbedding(theta, d_model // num_heads, max_seq_len), token_positions)
...
def run_transformer_lm(
    ...
    """
    transformer_lm = TransformerLm(d_model, num_heads, d_ff, vocab_size, context_length, num_layers, rope_theta, device=in_indices.device, dtype = in_indices.dtype)
